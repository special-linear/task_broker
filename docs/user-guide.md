...
Table sorting does not configure worker claims. Workers send their own sort, for example `client.claim(5, sort=[("n", "asc"), ("m", "asc")])`. Claims prioritize tasks with zero attempts in the requesting profile since its last full reset; retries follow by oldest last grant, then the requested fields. Profile filters, attempt limits and active leases can also make claim order differ from the global table. Reset affects future claims; existing claim receipts replay their original grants.
