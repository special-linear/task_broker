...
1. **Public repository installation:** the source repository URL has not been supplied. The same repository should contain source, the Deploy button and Releases. `scripts/deploy-link.mjs` produces links from that URL. A fresh browser-only installation through a published template and an actual GitHub Release are not yet verified.
